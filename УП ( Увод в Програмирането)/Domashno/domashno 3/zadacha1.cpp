...
int main(){
    ...
    while(tasksCount > 0){
            ...
            if(smallestNumberInUnsignedArray(minutes,tasksCount) > minutesWithoutMostWeightTask){
                ...
                if(sumPoints > points[mostWeightIndex]){
                    ...
                    for(int i=0; i<j; i++){
                        ...
                        if(!removeFromAllArrays(indexesOfComb[i], tasksCount, indexes, minutes, points, weight)){
                            delete[] indexes;
                            delete[] minutes;
                            delete[] points;
                            delete[] weight;
        ...
        if(!removeFromAllArrays(mostWeightIndex, tasksCount, indexes, minutes, points, weight)){
            delete[] indexes;
            delete[] minutes;
            delete[] points;
            delete[] weight;
