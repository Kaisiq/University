...
void sortByWeight(Fruit arr[], int len){
    for (int i=0; i<len; i++){
        int min = i;
        for(int j=i+1; j<len; j++){
            if((strcmp(arr[min].name,arr[j].name) == 0)  &&  (arr[min].weight>arr[j].weight)){
                min = j;
            }
        }
        swap(arr[min],arr[i]);
...
void sortByPrice(Fruit arr[], int len){
        for (int i=0; i<len; i++){
            int min = i;
            for(int j=i+1; j<len; j++){
                if((strcmp(arr[min].name,arr[j].name) == 0)  &&  (arr[min].weight==arr[j].weight)  &&  arr[min].price>arr[j].price){
                    min = j;
                }
            }
            swap(arr[min],arr[i]);
